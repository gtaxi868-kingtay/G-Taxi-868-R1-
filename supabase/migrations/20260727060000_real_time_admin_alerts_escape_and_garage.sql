-- ═══════════════════════════════════════════════════════════════════
-- REAL-TIME ADMIN ALERTS: G-ESCAPE TIPPING POINTS + G GARAGE (2026-07-27)
--
-- Confirmed real gap (audited before writing this): escape_sweep_tipping_points()
-- already does the right thing — files a real g_proposed_actions proposal
-- instead of blindly auto-confirming, with a genuine 24h-before-departure
-- safety net. But nothing ever told an admin it happened. The only way to
-- see it was to manually open Approvals.tsx, which itself only loads once
-- on mount. That means the "admin gets a real window to make the real
-- booking with the real vendor" claim was true in theory, false in
-- practice — an admin with the app closed would never know a booking
-- window opened until they happened to check.
--
-- This wires the missing half: a real push alert (via the new
-- notify_admins edge function) at the exact moment a proposal is filed,
-- for both the G-Escape tipping-point flow and G Garage requests. It
-- also fixes expires_at on escape proposals: it was defaulting to a
-- generic 72h (g_proposed_actions' table default), completely unrelated
-- to the REAL vendor deadline (departure_time minus 24h) already computed
-- elsewhere in this same function — now it's set to the real number.
-- ═══════════════════════════════════════════════════════════════════
...
-- ─── G Garage: same real-time alert, driver's request → admin's phone ──
...
BEGIN
    ...
    PERFORM net.http_post(
        ...
        body := jsonb_build_object(
            ...
            'body', format('%s requested a %s (%s) — review in G Garage.', COALESCE(v_driver_name, 'A driver'), NEW.vehicle_class_key, NEW.source),
...
DROP TRIGGER IF EXISTS trg_notify_admins_new_garage_request ON public.g_garage_requests;
CREATE TRIGGER trg_notify_admins_new_garage_request
    AFTER INSERT ON public.g_garage_requests
    FOR EACH ROW
    EXECUTE FUNCTION public.notify_admins_new_garage_request();
