...
-- For drivers to see rider info, or strict it? Let's leave public for name display.
...
CREATE POLICY "Public read drivers" ON public.drivers FOR SELECT TO authenticated USING (true);
-- Authenticated users can see drivers (for map)
...
CREATE POLICY "Authenticated read locations" ON public.driver_locations FOR SELECT TO authenticated USING (true);
-- For riders to see ghosts
