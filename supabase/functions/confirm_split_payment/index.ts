  ...
  try {
    ...
    for (const p of participants) {
      ...
      if (wallet && wallet.balance_cents >= p.amount_cents) {
        // Debit the participant's share. The wallet-balance sync trigger
        // (sync_wallet_balance) decrements wallets.balance_cents automatically.
        const { error: deductErr } = await supabaseAdmin
          .from("wallet_transactions")
          .insert({
            user_id: p.user_id,
            ride_id: session.ride_id || null,
            amount: -p.amount_cents,
            transaction_type: "split_payment",
            description: `Group split — ${session.title}`,
            status: "completed",
          });
