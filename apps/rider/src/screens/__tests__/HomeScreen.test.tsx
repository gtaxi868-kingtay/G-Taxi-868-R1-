...
jest.mock('@gtaxi/core', () => {
  ...
  const supabaseMock = { channel: () => channelObj, removeChannel: jest.fn(), from: () => ({ select: () => ({ eq: () => eqResult, order: () => ({ limit: () => Promise.resolve({ data: [], error: null }) }), then: (cb: any, errCb: any) => Promise.resolve({ data: null, error: null }).then(cb, errCb) }) }), functions: { invoke: jest.fn(() => Promise.resolve({ data: null, error: null })) }, rpc: jest.fn(() => rpcResult) };
