...
jest.mock('@gtaxi/core', () => ({ supabase: { channel: () => ({ on: () => ({ subscribe: jest.fn() }) }), from: () => ({ select: () => ({ eq: () => ({ single: jest.fn(), maybeSingle: jest.fn(), order: () => ({ limit: () => ({ data: null }) }) }), then: (cb: any, errCb: any) => Promise.resolve({ data: null, error: null }).then(cb, errCb) }) }) } }));
...
describe('PromoScreen', () => {
  it('renders without crashing', () => {
    ...
    const { getByText } = render(<PromoScreen navigation={navigation as any} />);
