...
jest.mock('@gtaxi/core', () => ({ supabase: { channel: () => ({ on: () => ({ subscribe: jest.fn() }) }), from: () => ({ select: () => ({ eq: () => ({ single: jest.fn(), maybeSingle: jest.fn(), order: () => ({ limit: () => ({ data: null }) }) }) }) }), functions: { invoke: jest.fn() }, rpc: jest.fn(), removeChannel: jest.fn() }, auth: { getSession: jest.fn() } }));
