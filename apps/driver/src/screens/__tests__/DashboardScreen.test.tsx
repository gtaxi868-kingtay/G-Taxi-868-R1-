...
jest.mock('@gtaxi/core', () => ({ supabase: { channel: () => ({ on: () => ({ subscribe: jest.fn() }) }), from: () => ({ select: () => ({ eq: () => ({ single: jest.fn(), maybeSingle: jest.fn(), order: () => ({ limit: () => ({ data: null }) }) }) }) }), functions: { invoke: jest.fn() }, rpc: jest.fn(() => ({ then: (cb: any, errCb: any) => Promise.resolve({ data: null, error: null }).then(cb, errCb) })), removeChannel: jest.fn() }, auth: { getSession: jest.fn() }, OutboxService: { getInstance: () => ({ enqueue: jest.fn(), process: jest.fn(), getPendingCount: jest.fn(), clear: jest.fn(), start: jest.fn(), stop: jest.fn() }) } }));
jest.mock('@g868/shared/env', () => ({ DEFAULT_LOCATION: { latitude: 10.6918, longitude: -61.2225 }, ENV: { MAPBOX_PUBLIC_TOKEN: 'test' } }));
